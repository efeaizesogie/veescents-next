...
async function getSections() {
  ...
  const sections = await Section.find().sort({ order: 1, name: 1 }).lean() as Array<{
    _id: { toString(): string };
    slug: string;
    name: string;
    description?: string;
  ...
  const withCounts = await Promise.all(
    sections.map(async (s) => {
      ...
      const sample = await Product.findOne({ section: s.slug }).lean() as { image?: string } | null;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
          {[
            ...
            <Link key={item.label} href={item.href}
              ...
              <h3 className="font-serif text-lg text-accent-dark mb-1.5 group-hover:text-accent-gold transition-colors">{item.label}</h3>
            ...
            return (
              <Link key={s._id.toString()} href={`/sections/${s.slug}`}
                ...
                <div className="w-full md:w-[55%] p-5 md:px-8 md:py-6">
                  ...
                  <h2 className="font-serif text-2xl md:text-3xl text-accent-dark mb-2">{s.name}</h2>
                  {s.description && <p className="text-gray-600 text-sm mb-4">{s.description}</p>}
