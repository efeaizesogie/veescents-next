...
export default function SectionProductsClient({ section }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    else if (sort === 'best_selling') out.sort((a, b) => (b.salesCount ?? 0) - (a.salesCount ?? 0));
    else if (sort === 'newest') out.sort((a, b) => (a.isNewProduct === b.isNewProduct ? 0 : a.isNewProduct ? -1 : 1));
    return out;
  ...
  return (
    <div className="pt-32 pb-20 bg-cream-50 min-h-screen">
      <div className="container mx-auto px-6">
        <Link href="/sections" className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-gray-400 hover:text-accent-gold transition-colors mb-8">
