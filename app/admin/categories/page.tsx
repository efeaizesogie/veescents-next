            ...
            <tbody className="divide-y divide-gray-50">
              {categories.map(cat => (
                <tr key={cat._id} className={`hover:bg-cream-50 transition-colors ${editing === cat._id ? 'bg-accent-gold/5' : ''}`}>
                  ...
                  <td className="px-6 py-4 hidden md:table-cell text-gray-400 text-xs">{cat.description || '—'}</td>
                  <td className="px-6 py-4 hidden sm:table-cell text-gray-500">{cat.order}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => openEdit(cat)} className="p-2 hover:text-accent-gold transition-colors text-gray-400">
                        <Pencil size={15} />
                      </button>
                      <button onClick={() => remove(cat._id)} className="p-2 hover:text-red-500 transition-colors text-gray-400">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </td>
