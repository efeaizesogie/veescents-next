  ...
  return (
    <div className="page-shell bg-cream-50 min-h-screen">
      <div className="container mx-auto px-6">
        <SectionTitle
          title="Categories"
          subtitle="Browse by category with quick discovery tools."
        />

        {loadingCats ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="h-28 rounded-sm bg-white animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-10">
            {categoryTabs.map((cat) => {
              ...
              return (
                <button
                  ...
                  className={`relative h-28 overflow-hidden rounded-sm border text-left ${
                    active ? "border-accent-gold" : "border-gray-100"
                  } group`}
                >
                  {cat.image ? (
                    <Image
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              {currentCategory?.description ||
                "Browse perfumes in this category."}
            </p>
          </div>
